      ...
      <div class="flex items-center gap-4">
        ...
        {#if $isConnected}
          <button
            ...
            disabled={isReadingTechQuality || $isMeterReading}
          ...
          >
            <Icon name="speed" size="sm" class={isReadingTechQuality ? "animate-spin-reverse" : ""} />
            {isReadingTechQuality ? $t.readingPacket : $t.readTechQuality}
          </button>
          <button
            onclick={refreshMeasurements}
            disabled={isRefreshing || $isMeterReading}
